//
//  PitchEffectExtensionDSPKernel.hpp
...
#import "PitchEffectExtension-Swift.h"
#import "PitchEffectExtensionParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 PitchEffectExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class PitchEffectExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in PitchEffectExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case PitchEffectExtensionParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case PitchEffectExtensionParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<PitchEffectExtensionDSPKernel *>(context);
